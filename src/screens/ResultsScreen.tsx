...
export function ResultsScreen({ routes }: ResultsScreenProps) {
  ...
  const selectedRoute =
    routes.find((route) => route.providerRouteId === selectedRouteId) ?? null;
  ...
  return (
    <section
      ...
      data-presentation={resultsPresentation}
    >
      <div className="section-headingRow results-screen__header">
        <div>
          <h2 className="section-heading">경로 추천</h2>
          <p className="results-screen__meta">
            도착 전 알림에 가장 적합한 대중교통 경로를 골라보세요.
          </p>
        </div>
      </div>
      {routes.length === 0 ? (
        ...
        <div className="results-layout">
          ...
          {selectedRoute ? (
            <RouteDetailCard
              route={selectedRoute}
              isExpanded={resultsPresentation === "detail_expanded"}
